...
if uploaded_file:
    ...
    for i in range(len(signal) // segment_length):
        ...
        # Handle zero or corrupt segments
        if np.max(spectrogram) == 0 or np.isnan(spectrogram).any():
            continue

        spectrogram = (spectrogram - np.min(spectrogram)) / (np.max(spectrogram) - np.min(spectrogram) + 1e-8)
        try:
            spectrogram = torch.tensor(spectrogram).unsqueeze(0).unsqueeze(0).to(device)
        except Exception as e:
            st.warning(f"⚠️ Skipped a corrupt segment: {e}")
            continue
    ...
    if predictions:
        ...
        st.error("🚫 No valid segments to classify! Possibly due to high noise or corrupted signal.")
